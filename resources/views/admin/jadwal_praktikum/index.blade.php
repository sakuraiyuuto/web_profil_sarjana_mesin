    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Jadwal Praktikum</h1>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Data Jadwal Praktikum</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <div class="row">
                                ...
                                <div class="table-responsive">
                                    <table id="tabel_jadwal_praktikum" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Nomor</th>
                                                <th>Nama</th>
                                        ...
                                        <tbody>
                                            @foreach ($jadwalPraktikums as $jadwalPraktikum)
                                                <tr>
                                                    ...
                                                    <td><a href="{{ url($jadwalPraktikum->nama_file) }}" download
                                                            target="_blank">{{ $jadwalPraktikum->nama_file }}</a></td>
                                                    <td>{{ $jadwalPraktikum->release_date }}</td>
                                                    @if ($jadwalPraktikum->deleted_at == '')
                                                        @if ($jadwalPraktikum->release_date <= date('Y-m-d'))
                                                            <td>Rilis</td>
                                                        @else
                                                            <td>Belum Rilis</td>
                                                        @endif
                                                    ...
                                                    @if ($jadwalPraktikum->deleted_at == '')
                                                        <td>
                                                            <button href="#formModalEdit" role="button" data-toggle="modal"
                                                                data-id="{{ $jadwalPraktikum->id }}"
                                                                data-nama="{{ $jadwalPraktikum->nama }}"
                                                                data-semester="{{ $jadwalPraktikum->semester }}"
                                                                data-tahun_ajaran="{{ $jadwalPraktikum->tahun_ajaran }}"
                                                                data-nama_file="{{ $jadwalPraktikum->nama_file }}"
                                                                data-release_date="{{ $jadwalPraktikum->release_date }}"
                                                                class="btn btn-warning open-formModalEdit"><i
                                                                    class="fa fa-edit"></i> Edit</button>
                                                            <form
                                                                action="{{ route('jadwal_praktikum.destroy', $jadwalPraktikum) }}"
                                                                method="post">
                                                                @method('delete')
                                                                @csrf
                                                    ...
                                                    @else
                                                        <td>
                                                            <form
                                                                action="{{ url('/admin/jadwal_praktikum/' . $jadwalPraktikum->id . '/restore') }}"
                                                                method="post">
                                                                @csrf
                                                            ...
                                                            <form
                                                                action="{{ url('/admin/jadwal_praktikum/' . $jadwalPraktikum->id . '/delete') }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit" class="btn btn-danger"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="formModalLabel">Tambah Data Jadwal Praktikum</h5>
                    <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                </div>
                <form id="formAdd" action="{{ url('/admin/jadwal_praktikum') }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group mt-2">
                            <label for="nama">Nama</label>
                            <input type="text" class="form-control" name="nama" placeholder="Masukkan Judul" value=""
                        ...
                        <div class="form-group">
                            <label for="nama_file">File Jadwal Praktikum</label>
    ...
    @foreach ($jadwalPraktikums as $jadwalPraktikum)
        ...
        <div class="modal fade" id="formModalEdit" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="formModalLabel">Ubah Data Jadwal Praktikum</h5>
                        <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                    </div>
                    <form id="formEdit" action="{{ route('jadwal_praktikum.update', $jadwalPraktikum->id) }}"
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group mt-2">
                                <label for="nama">Nama</label>
                                <input type="text" class="form-control" id="nama" name="nama" placeholder="Masukkan Judul"
                            ...
                            <div class="form-group">
                                <label for="nama_file">File Jadwal Praktikum (Maksimal 2MB)</label>
...
@section('script')
    ...
    <script>
        $(function() {
            $("#tabel_jadwal_praktikum").DataTable({
                "paging": true,
                "lengthChange": false,
                "searching": true,
                "ordering": true,
            ...
            }).buttons().container().appendTo('#tabel_jadwal_praktikum_wrapper .col-md-6:eq(0)');
        });
