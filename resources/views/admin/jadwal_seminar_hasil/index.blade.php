...
@section('title', 'Jadwal Seminar Hasil')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Jadwal Seminar Hasil</h1>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Data Jadwal Seminar Hasil</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12 text-left">
                                        <a type="button" href="#formModalAdd" data-toggle="modal"
                                            ...
                                            Tambah Data Jadwal Seminar Hasil
                                        </a>
                                    </div>
                                </div>
                                <div class="table-responsive">
                                    <table id="tabel_jadwal_seminar_hasil" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                <th>Nomor</th>
                                                <th>Semester</th>
                                        ...
                                        <tbody>
                                            @foreach ($jadwalSeminarHasils as $jadwalSeminarHasil)
                                                <tr>
                                                    ...
                                                    <td>{{ $jadwalSeminarHasil->semester }}</td>
                                                    <td>{{ $jadwalSeminarHasil->tahun_ajaran }}</td>
                                                    <td><a href="{{ url($jadwalSeminarHasil->nama_file) }}" download
                                                            target="_blank">{{ $jadwalSeminarHasil->nama_file }}</a></td>
                                                    <td>{{ $jadwalSeminarHasil->release_date }}</td>
                                                    @if ($jadwalSeminarHasil->deleted_at == '')
                                                        @if ($jadwalSeminarHasil->release_date <= date('Y-m-d'))
                                                            <td>Rilis</td>
                                                        @else
                                                            <td>Belum Rilis</td>
                                                        @endif
                                                    ...
                                                    @if ($jadwalSeminarHasil->deleted_at == '')
                                                        <td>
                                                            <button href="#formModalEdit" role="button" data-toggle="modal"
                                                                data-id="{{ $jadwalSeminarHasil->id }}"
                                                                data-semester="{{ $jadwalSeminarHasil->semester }}"
                                                                data-tahun_ajaran="{{ $jadwalSeminarHasil->tahun_ajaran }}"
                                                                data-nama_file="{{ $jadwalSeminarHasil->nama_file }}"
                                                                data-release_date="{{ $jadwalSeminarHasil->release_date }}"
                                                                class="btn btn-warning open-formModalEdit"><i
                                                                    class="fa fa-edit"></i> Edit</button>
                                                            <form
                                                                action="{{ route('jadwal_seminar_hasil.destroy', $jadwalSeminarHasil) }}"
                                                                method="post">
                                                                @method('delete')
                                                                @csrf
                                                    ...
                                                    @else
                                                        <td>
                                                            <form
                                                                action="{{ url('/admin/jadwal_seminar_hasil/' . $jadwalSeminarHasil->id . '/restore') }}"
                                                                method="post">
                                                                @csrf
                                                            ...
                                                            <form
                                                                action="{{ url('/admin/jadwal_seminar_hasil/' . $jadwalSeminarHasil->id . '/delete') }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit" class="btn btn-danger"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="formModalLabel">Tambah Data Jadwal Seminar Hasil</h5>
                    <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                </div>
                <form id="formAdd" action="{{ url('/admin/jadwal_seminar_hasil') }}" method="post"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="nama_file">File Jadwal Seminar Hasil</label>
    ...
    @foreach ($jadwalSeminarHasils as $jadwalSeminarHasil)
        ...
        <div class="modal fade" id="formModalEdit" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="formModalLabel">Ubah Data Jadwal Seminar Hasil</h5>
                        <button type="button" class="btn btn-danger" data-dismiss="modal" aria-label="Close">X</button>
                    </div>
                    <form id="formEdit" action="{{ route('jadwal_seminar_hasil.update', $jadwalSeminarHasil->id) }}"
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="nama_file">File Jadwal Seminar Hasil (Maksimal 2MB)</label>
...
@section('script')
    ...
    <script>
        $(function() {
            $("#tabel_jadwal_seminar_hasil").DataTable({
                "paging": true,
                "lengthChange": false,
                "searching": true,
                "ordering": true,
            ...
            }).buttons().container().appendTo('#tabel_jadwal_seminar_hasil_wrapper .col-md-6:eq(0)');
        });
