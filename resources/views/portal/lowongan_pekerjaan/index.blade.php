...
@section('title', 'Lowongan Pekerjaan - Teknik Mesin UNTAN')
                ...
                <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="blog_3_wrap">
                                        ...
                                        <ul style="z-index : 1" class="blog_3_sidebar">
                                            <li>
                                                <a href="#">
                                                    {{ date('d', strtotime($lowonganPekerjaan->release_date)) }}
                                                    <span>
                                                        {{ substr(date('F', strtotime($lowonganPekerjaan->release_date)), 0, 3) }}
                                                    </span>
                                                </a>
                                            </li>
                                        </ul>
                                        ...
                                        <div class="blog_3_des">
                                            <figure>
                                                <img style="height : 20rem;object-fit:cover"
                                                    src="{{ url($lowonganPekerjaan->thumbnail) }}" alt="" />
                                                <figcaption><a href="{{ url($lowonganPekerjaan->slug) }}"><i
                                                            class="fa fa-search-plus"></i></a>
                                                </figcaption>
                                            </figure>
                                            <h5><a
                                                    href="{{ url($lowonganPekerjaan->slug) }}">{{ $lowonganPekerjaan->judul }}</a>
                                            </h5>
                                            <li><i class="fa fa-calendar"></i>
                                                {{ date(' Y', strtotime($lowonganPekerjaan->release_date)) }}</li>
                                            <p>
                                                @if (strlen(strip_tags($lowonganPekerjaan->teks)) > 256)
                                                    {{ str_replace('&nbsp;', '', substr(strip_tags($lowonganPekerjaan->teks), 0, 256) . '...') }}
                                                @else
                                                    {{ str_replace('&nbsp;', '', substr(strip_tags($lowonganPekerjaan->teks), 0, 256)) }}
                                                @endif
                                            </p>
                                            <a class="readmore" href="{{ url($lowonganPekerjaan->slug) }}">
                                                Selengkapnya
                    ...
                    <div class="kf_edu_pagination_wrap">
                        <ul class="pagination">
                            <div class="center">
                                <ul class="pagination">
                                    {{ $lowonganPekerjaans->links('pagination::default') }}
                                </ul>
                            </div>
                        </ul>
                    </div>
